...
function Home() {
  const {lang}=useLanguage();
  return <>
    <section className="hero" data-protocol={lang==="zh"?"系统在线 // CST_AI_倡议 // KAKU_协议_V1":"SYSTEM ONLINE // CST_AI_INITIATIVE // KAKU_PROTOCOL_V1"}>
      ...
      <div className="hero-visual"><Photo src="/assets/events/forum-air/cst-audience.jpg" alt={lang==="zh"?"参与 CST 社群活动的现场观众":"Audience participating at a CST community event"} className="tall" /><div className="live-note"><i /> {lang==="zh"?"下一场：Forum AIR #01 · 7月25日":"Next: Forum AIR #01 · 25 July"}</div></div>
    ...
    <section className="cinematic-band"><Photo src="/assets/events/forum-air/cst-keynote.jpg" alt={lang==="zh"?"CST 社群主题分享与学习现场":"CST community keynote and learning session"} /><div><span>{lang==="zh"?"人类转型协议":"HUMAN TRANSITION PROTOCOL"}</span><strong>{lang==="zh"?<>看见变化。<br/>确认位置。<br/>一起前进。</>:<>See the shift.<br/>Name your position.<br/>Move together.</>}</strong></div></section>
    ...
    <section className="events-section"><div className="section-head"><div><p className="eyebrow">{lang==="zh"?"近期活动":"UPCOMING"}</p><h2>{lang==="zh"?"加入这场共同对话。":"Step into the conversation."}</h2></div><a className="text-link light" href={localPath("/calendar",lang)}>{lang==="zh"?"查看完整活动日历":"View full calendar"} →</a></div><div className="event-grid">{events.slice(0,3).map(e=><EventCard key={e.title} e={e}/>)}</div></section>
    <section className="learning"><div className="section-head"><div><p className="eyebrow dark">{lang==="zh"?"在建构中学习":"LEARN BY BUILDING"}</p><h2>{lang==="zh"?<>实用能力。<br />深度思考。</>:<>Practical capability.<br />Deeper thinking.</>}</h2></div><p>{lang==="zh"?"从实用 AI 技能开始，逐步进入系统思维、决策智能与社群领导力。":"Start with useful AI skills. Grow into systems, decisions and community leadership."}</p></div>{(lang==="zh"?[["01","AI 基础","把实用 AI、生产力与提示思维应用到日常工作。"],["02","AI 工作流程","建立智能体、自动化、仪表板与可重复使用的商业系统。"],["03","AI 创作","运用 AI 进行图像、视频、内容与媒体制作。"],["04","氛围编程","建立网站、应用程序、游戏、原型与实用内部工具。"],["05","AI 领导力","学习系统、杠杆、决策智能与自组织。"]]:learning).map(x=><a href={localPath("/workshops",lang)} className="learning-row" key={x[0]}><span>{x[0]}</span><h3>{x[1]}</h3><p>{x[2]}</p><b>↗</b></a>)}</section>
    <MembershipStrip />
    <section className="pulse-preview"><div><p className="eyebrow dark">{lang==="zh"?"AI 动态 · 智能新闻室":"AI PULSE · AGENTIC NEWSROOM"}</p><h2>{lang==="zh"?"了解发生了什么变化，以及为什么与你有关。":"Understand what changed—and why it matters."}</h2><p>{lang==="zh"?"搜集 → 核实 → 分类 → 排序 → 解读 → 发布 → 根据反馈持续学习。":"Gather → verify → cluster → rank → explain → publish → learn from feedback."}</p><a className="button dark-button" href={localPath("/pulse",lang)}>{lang==="zh"?"探索 AI 动态":"Explore AI Pulse"} →</a></div><div className="news-card"><span>{lang==="zh"?"今日 · 示范资讯流":"TODAY · PLACEHOLDER FEED"}</span><h3>{lang==="zh"?"把重要的 AI 发展，转化为日常生活听得懂的内容。":"Top AI developments, translated for real life."}</h3><ul>{(lang==="zh"?["发生了什么","为什么重要","对中小企业、专业人士与社群意味着什么","消息来源与可信度"]:["What happened","Why it matters","What it means for SMEs, professionals and communities","Sources and confidence"]).map(x=><li key={x}>{x}</li>)}</ul></div></section>
  </>;
}
