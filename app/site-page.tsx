...
const AI_PROMPT_THINKING_URL = "https://annabong.github.io/cst-impact-academy/ai-prompt-thinking-workshop/";
...
const pageData: Record<string, { eyebrow: [string,string]; title: [string,string]; lead: [string,string] }> = {
  ...
  "ai-prompt-thinking": { eyebrow: ["KAKU 365 • 3 HOURS • REPEATABLE PRACTICE","KAKU 365 • 3 小时 • 可持续复训"], title: ["Prompt better. Think deeper. Work with AI.","不只会问 AI，更要学会与 AI 一起思考。"], lead: ["A practical foundation in prompting, meta-prompting and iterative AI thinking—built for beginners, professionals, founders and everyday work.","从提示、元提示到迭代思维，为新手、专业人士、创业者与日常工作建立真正可重复使用的 AI 基础。"] },
  ...
  about: { eyebrow: ["THE GREYBEARD MOMENT","灰胡子时刻"], title: ["In a new era, we are beginners again.","新时代里，我们再次成为初学者。"], lead: ["Jane Goodall saw intelligence where others saw the primitive. David Greybeard showed us that evolution begins with awareness, learning and adaptation.","珍·古道尔在「原始」之中看见智慧。大卫·灰胡子提醒我们：进化始于觉察、学习与适应。"] },
};
...
const events: CalendarEvent[] = [
  ...
  { category: "AI for Skills", iso: "2026-08-07", date: "07 AUG", time: "7:00–10:00 PM", title: "AI Prompt Thinking Workshop", copy: "Move from chat and prompts to loops, meta-prompting and reusable AI thinking.", href: AI_PROMPT_THINKING_URL },
  { category: "AI for Skills", iso: "2026-08-08", date: "08 AUG", time: "9:00 AM–12:00 PM", title: "AI Prompt Thinking Workshop", copy: "Move from chat and prompts to loops, meta-prompting and reusable AI thinking.", href: AI_PROMPT_THINKING_URL },
  ...
  { category: "AI for Skills", iso: "2026-08-22", date: "22 AUG", time: "9:00 AM–12:00 PM", title: "AI Prompt Thinking Workshop", copy: "Move from chat and prompts to loops, meta-prompting and reusable AI thinking.", href: AI_PROMPT_THINKING_URL },
...
function EventCard({e}:{e:typeof events[number]}) {
 ...
 return <article className="event-card"><span className="tag">{shown.category}</span><div className="event-date"><strong>{shown.date}</strong><span>{shown.time}</span></div><h3>{shown.title}</h3><p>{shown.copy}</p><a href={destination||localPath("/calendar",lang)} target={isExternal?"_blank":undefined} rel={isExternal?"noreferrer":undefined}>{isForum?(lang==="zh"?"立即报名":"Register now"):isPromptThinking?(lang==="zh"?"查看课程内容":"View workshop outline"):e.href?(lang==="zh"?"认识合作伙伴":"Visit partner"):(lang==="zh"?"查看详情":"Details")} ↗</a></article>
}

function MembershipStrip() {
 const {lang}=useLanguage();
 return <section className="membership-strip"><div><p className="eyebrow">KAKU {lang==="zh"?"会员计划":"MEMBERSHIP"}</p><h2>{lang==="zh"?"365 天，持续学习、应用、连接与成长。":"365 days to learn, apply, connect and grow."}</h2><p>{lang==="zh"?"加入持续学习社群，或选择完整的领导力学习路径。":"Join the continuous learning community—or take the complete leadership learning pathway."}</p></div><article><span>KAKU 365</span><strong>RM365<small>/ {lang==="zh"?"年":"year"}</small></strong><p>{lang==="zh"?"一年会员期内，可按公布日历与席位安排，无限次参加 AI 提示思维工作坊。":"Unlimited AI Prompt Thinking workshop attendance for one membership year, subject to the published calendar and seat availability."}</p><div className="membership-actions"><a className="button white" href={KAKU_365_PAYMENT_URL} target="_blank" rel="noreferrer">{lang==="zh"?"立即注册与付款":"Register & pay"} →</a><a className="text-link" href={localPath("/ai-prompt-thinking",lang)}>{lang==="zh"?"查看课程内容":"View workshop outline"} ↗</a></div></article><article className="featured"><span>KAKU GO</span><strong>RM1,825<small>/ {lang==="zh"?"年":"year"}</small></strong><p>{lang==="zh"?"包含 Kaku 365 全部权益，并可进入系统思维、杠杆、决策与自组织领导力课程。":"Everything in Kaku 365, plus the Systems, Leverage, Decision and Self-Organisation leadership pathway."}</p><div className="membership-actions"><a className="button" href={withBase("/programs/thinking-in-systems.html")} target="_blank" rel="noreferrer">{lang==="zh"?"查看领导力课程":"View leadership programme"} ↗</a><a className="text-link" href={localPath("/membership",lang)}>{lang==="zh"?"比较会员权益":"Compare membership"} →</a></div></article></section>;
}
...
function Membership() {
 const {lang}=useLanguage();
 return <><InnerHero page="membership"/><MembershipStrip/><section className="journey"><div><p className="eyebrow dark">{lang==="zh"?"会员体验":"MEMBER EXPERIENCE"}</p><h2>{lang==="zh"?"从加入，到成为贡献者。":"From joining to contributing."}</h2></div><div className="loop">{(lang==="zh"?["加入","入门","学习","应用","分享","带领"]:["Join","Onboard","Learn","Apply","Share","Lead"]).map((x,i)=><div key={x}><span>{i+1}</span>{x}</div>)}</div></section><section className="comparison"><h2>{lang==="zh"?"两种进入 Kaku 生态系统的方式。":"Two ways to enter the ecosystem."}</h2><div><article><span>KAKU 365</span><h3>{lang==="zh"?"持续学习":"Continuous learning"}</h3><ul>{(lang==="zh"?["365 天会员资格","Kaku AI 会员入门","每月学习交流活动","会员资源库","社群活动","一年内按日历无限次参加 AI 提示思维工作坊"]:["365-day membership","Kaku AI Onboarding","Monthly learning touchpoints","Resource library","Community activities","Unlimited scheduled AI Prompt Thinking attendance for one year"]).map(x=><li key={x}>{x}</li>)}</ul><div className="membership-actions"><a className="button dark-button" href={KAKU_365_PAYMENT_URL} target="_blank" rel="noreferrer">{lang==="zh"?"注册并支付 RM365":"Register & pay RM365"} →</a><a className="text-link" href={localPath("/ai-prompt-thinking",lang)}>{lang==="zh"?"查看 AI 提示思维课程":"View AI Prompt Thinking"} ↗</a></div></article><article className="accent-card"><span>KAKU GO</span><h3>{lang==="zh"?"完整领导力学习路径":"Complete leadership pathway"}</h3><ul>{(lang==="zh"?["包含 Kaku 365 的全部权益","AI 提示思维","系统思维与杠杆","决策智能","社群与自组织","建设者与讲者成长路径"]:["Everything in Kaku 365","AI Prompt Thinking","Systems & Leverage","Decision Intelligence","Community & Self-Organisation","Builder and speaker pathways"]).map(x=><li key={x}>{x}</li>)}</ul><a className="button" href={withBase("/programs/thinking-in-systems.html")} target="_blank" rel="noreferrer">{lang==="zh"?"查看完整课程内容与大纲":"View programme content & outline"} ↗</a></article></div></section></>;
}

function AiPromptThinking() {
 const {lang}=useLanguage();
 const modules=lang==="zh"?[
  ["01","语言就是界面","理解角色、语境、动词、约束与输出格式，如何共同影响 AI 的回应。"],
  ["02","Chat → Prompt → Loop","从随意聊天进阶到结构化提示，再通过检查、反馈与再提示持续改善结果。"],
  ["03","Meta-Prompting 元提示","让 AI 反过来向你提问、指出资料缺口、挑战假设，并协助设计更好的提示。"],
  ["04","1D · 2D · 3D 思维","从单点答案扩展到不同维度、观点与关系，再重新聚焦真正重要的部分。"],
  ["05","语音与实用工作流","运用 Wispr Flow 等语音工具，把想法快速转化成结构、内容与可执行步骤。"],
  ["06","建立个人提示系统","整理可以重复使用的关键词、检查清单与提示模板，让 AI 成为日常习惯。"],
 ]:[
  ["01","Language is the interface","See how roles, context, verbs, constraints and output formats shape an AI response."],
  ["02","Chat → Prompt → Loop","Move from casual chat to structured prompting, then improve results through checking, feedback and re-prompting."],
  ["03","Meta-Prompting","Ask AI to question you, expose missing context, challenge assumptions and help design a stronger prompt."],
  ["04","1D · 2D · 3D Thinking","Expand one answer into dimensions, perspectives and relationships—then focus on what matters."],
  ["05","Voice and practical workflow","Use voice tools such as Wispr Flow to turn raw thinking into structure, content and next actions."],
  ["06","Your personal prompt system","Build reusable keywords, checklists and prompt patterns that make AI part of an everyday habit."],
 ];
 const loop=lang==="zh"?["表达","提示","检查","反思","再提示","应用"]:["Express","Prompt","Check","Reflect","Re-prompt","Apply"];
 return <><InnerHero page="ai-prompt-thinking"/>
  <section className="apt-positioning"><div><p className="eyebrow dark">{lang==="zh"?"不只是提示工程":"MORE THAN PROMPT ENGINEERING"}</p><h2>{lang==="zh"?"工具会改变，思考与表达的能力会留下。":"Tools change. Your ability to think and articulate remains."}</h2></div><div><p>{lang==="zh"?"AI 提示思维不是背诵一百个提示词，而是学习如何表达语境、厘清目标、展开观点、检查结果，并与 AI 形成持续改善的思考循环。":"AI Prompt Thinking is not about memorising a hundred prompts. It is the practice of expressing context, clarifying goals, widening perspectives, checking results and building an iterative thinking loop with AI."}</p><p>{lang==="zh"?"适合刚开始使用 AI 的个人、专业人士、自由工作者、创业者与企业领导者。无需编程基础。":"Designed for beginners, professionals, freelancers, founders and business leaders. No coding experience is required."}</p></div></section>
...
function About() {
 const {lang}=useLanguage();
 return <>
  <section className="about-key-transition about-key-hero"><video className="about-key-background" autoPlay muted loop playsInline preload="metadata" aria-label={lang==="zh"?"Ka-Ku 世界之门动画":"Ka-Ku gateway animation"}><source src={withBase("/assets/about/NFT key animated gif.mp4")} type="video/mp4"/></video><div className="about-key-copy"><p className="eyebrow">{lang==="zh"?"你的进化，从觉察开始":"EVOLUTION BEGINS WITH AWARENESS"}</p><h1>{lang==="zh"?"进入 Ka-Ku 的世界":"Enter the world of Ka-Ku"}</h1><p className="key-line">{lang==="zh"?"一把开启无限可能的钥匙。":"Your key to every possibility."}</p><a className="button" href="#why-kaku">{lang==="zh"?"为什么是 Ka-Ku？":"Why Ka-Ku?"} ↓</a></div></section>
  <InnerHero page="about"/>
  <section className="statement about-awakening"><p className="eyebrow dark">{lang==="zh"?"AI 觉察时刻":"THE AI AWARENESS MOMENT"}</p><h2>{lang==="zh"?<>环境改变了。我们都要<em>重新学习。</em></>:<>The environment changed. We all have to <em>learn again.</em></>}</h2><p>{lang==="zh"?"AI 让熟悉的世界变得陌生。真正的优势，不是已经知道多少，而是能否保持觉察、快速学习，并与他人一起进化。":"AI has made the familiar world unfamiliar. The real advantage is not what we already know, but how well we notice, learn and evolve together."}</p></section>
  <section id="why-kaku" className="kaku-origin"><div className="origin-art"><img src={withBase("/assets/brand/kaku-identity.png")} alt="Chimpanzee Ka-Ku identity protocol"/><span>CHIMPANZEE KA-KU SOCIETY</span></div><div><p className="eyebrow">{lang==="zh"?"名字背后的故事":"WHY THE NAME KA-KU"}</p><h2>{lang==="zh"?"原始，不代表落后。":"Primitive does not mean powerless."}</h2><p>{lang==="zh"?"当珍·古道尔观察大卫·灰胡子使用工具，她改变了人类对黑猩猩的理解：它们会观察、尝试、学习，也会把经验传递给群体。":"When Jane Goodall observed David Greybeard using tools, she changed how humanity understood chimpanzees: they observe, experiment, learn and pass knowledge through the group."}</p><p>{lang==="zh"?"AI 时代到来，我们也像走进陌生世界的原始人。Ka-Ku 是那个觉察时刻：承认环境已经改变，并选择保持好奇、敏锐、学习与连接。":"As the AI era arrives, we are like early humans entering an unfamiliar world. Ka-Ku is the moment of awareness: recognising that the environment has changed—and choosing curiosity, alertness, learning and connection."}</p><blockquote>{lang==="zh"?"看见自己身处何处，才能决定如何进化。":"See where you are. Then choose how to evolve."}</blockquote></div></section>
  <section className="origin-principles">{(lang==="zh"?[["保持好奇","主动观察新的工具与可能。"],["保持敏锐","看见变化，也看见真正重要的信号。"],["保持学习","在实践、试错与反思中持续更新。"],["保持连接","把经验带回社群，与伙伴共同成长。"]]:[["Stay curious","Observe new tools and possibilities."],["Stay alert","Notice change—and the signals that truly matter."],["Keep learning","Update through practice, experimentation and reflection."],["Stay connected","Bring experience back and grow with others."]]).map((x,i)=><article key={x[0]}><span>0{i+1}</span><h3>{x[0]}</h3><p>{x[1]}</p></article>)}</section>
  <section className="about-grid about-path"><article><span>{lang==="zh"?"觉察":"01 · AWARE"}</span><h2>{lang==="zh"?"看见变化。":"See the change."}</h2><p>{lang==="zh"?"理解 AI 如何重塑工作、学习与生活。":"Understand how AI is reshaping work, learning and life."}</p></article><article><span>{lang==="zh"?"进化":"02 · EVOLVE"}</span><h2>{lang==="zh"?"一起前行。":"Move together."}</h2><p>{lang==="zh"?"通过实践、分享与社群连接，建立面向未来的能力。":"Build future-ready capability through practice, sharing and community."}</p></article></section>
  <section className="about-society"><img src={withBase("/assets/about/ChatGPT Image Sep 9, 2026, 11_15_18 PM.png")} alt={lang==="zh"?"65 位 Ka-Ku Society 成员":"65 Ka-Ku Society members"}/><div className="about-society-copy"><p className="eyebrow">KA-KU SOCIETY // LIVE</p><p className="society-count">435 / 500</p><h2>{lang==="zh"?"Ka-Ku 会员——倒数进行中":"Ka-Ku members — counting down"}</h2><div className="society-lines"><p>{lang==="zh"?"任何时代，一起前行。":"Navigating any era, together."}</p><p>{lang==="zh"?"进入未来社会——不再独自一人。":"Enter the future society — not alone."}</p><p>{lang==="zh"?"未来适应模式：开启。":"Future-adaptive mode: ON."}</p></div></div></section>
  <section className="about-join"><p className="eyebrow">{lang==="zh"?"你的下一步":"YOUR NEXT STEP"}</p><h2>{lang==="zh"?"不要独自面对未来。":"Do not navigate the future alone."}</h2><p>{lang==="zh"?"加入 Ka-Ku，与一群保持觉察、持续学习的人一起理解 AI、实践新能力，并共同进化。":"Join Ka-Ku to understand AI, practise new capabilities and evolve with a community that keeps learning."}</p><div className="actions"><a className="button" href={localPath("/membership",lang)}>{lang==="zh"?"加入 Ka-Ku":"Join Ka-Ku"} →</a><a className="text-link light" href={localPath("/community",lang)}>{lang==="zh"?"认识我们的社群":"Meet the community"} ↗</a></div></section>
 </>;
}
